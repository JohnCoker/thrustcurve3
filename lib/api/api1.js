...
function searchQuery(request, cache, error, strict) {
  ...
  // Optional error check for invalid criteria. This is only applicable for the
  // search verb since it gives false positives for other verbs that use search,
  // but also have other properties in the input.
  if (strict) {
    Object.keys(request).forEach(p => {
      if (p == "$" || // turd from XML parsing
          Array.isArray(strict) && strict.indexOf(p) >= 0)
        return;
...
module.exports = {
   ...
   * @param {object} error error collector
   * @param {boolean|string[]} [strict] if truthy, raise error for unknown criteria
