...
describe('api1', function() {
  ...
  describe("searchQuery", function() {
    ...
    it("id", function() {
      expect(api1.searchQuery({ id: "abcdef000000abcdef123456" }, cache, errors.throw, true)).toEqual({
        _id: "abcdef000000abcdef123456",
      });
      let errs = new errors.Collector();
      expect(api1.searchQuery({ id: "12" }, cache, errs)).toEqual({
        _id: null,
      });
      expect(errs.errorCount()).toBe(1);
      expect(errs.lastError().message).toBe('Invalid id value "12".');
    });
    it("manufacturer", function() {
      expect(api1.searchQuery({ manufacturer: "Estes" }, cache, errors.throw, true)).toEqual({
    ...
    it("designation", function() {
      expect(api1.searchQuery({ designation: "D12" }, cache, errors.throw, true)).toEqual({
    ...
    it("commonName", function() {
      expect(api1.searchQuery({ "common-name": "D12" }, cache, errors.throw, true)).toEqual({
        $or: [ { commonName: "D12" }, { altName: "D12" } ]
      });
      expect(api1.searchQuery({ commonName: "D12" }, cache, errors.throw, true)).toEqual({
        $or: [ { commonName: "D12" }, { altName: "D12" } ]
      });
      let errs = new errors.Collector();
    ...
    it("commonName and designation", function() {
      expect(api1.searchQuery({ "common-name": "F12", designation: "F12J" }, cache, errors.throw, true)).toEqual({
        $and: [
    ...
    it("impulseClass", function() {
      expect(api1.searchQuery({ "impulse-class": "D" }, cache, errors.throw, true)).toEqual({
        impulseClass: "D",
      });
      expect(api1.searchQuery({ impulseClass: "D" }, cache, errors.throw, true)).toEqual({
        impulseClass: "D",
      });
      let errs = new errors.Collector();
    ...
    it("diameter", function() {
      expect(api1.searchQuery({ diameter: "54" }, cache, errors.throw, true)).toEqual({
    ...
    it("type", function() {
      expect(api1.searchQuery({ type: "SU" }, cache, errors.throw, true)).toEqual({
        type: 'SU',
      });
      let errs = new errors.Collector();
    ...
    it("certOrg", function() {
      expect(api1.searchQuery({ "cert-org": "NAR" }, cache, errors.throw, true)).toEqual({
        _certOrg: "org000000000000000000001",
      });
      expect(api1.searchQuery({ certOrg: "NAR" }, cache, errors.throw, true)).toEqual({
        _certOrg: "org000000000000000000001",
      });
      let errs = new errors.Collector();
    ...
    it("sparky", function() {
      expect(api1.searchQuery({ sparky: "true" }, cache, errors.throw, true)).toEqual({
        sparky: true,
      });
      expect(api1.searchQuery({ sparky: false }, cache, errors.throw, true)).toEqual({
        sparky: false,
      });
      let errs = new errors.Collector();
    ...
    it("info-updated-since", function() {
      expect(api1.searchQuery({ "info-updated-since": "2020-01-01" }, cache, errors.throw, true)).toEqual({
        updatedAt: { $gte: new Date(2020, 0, 1) },
      });
      expect(api1.searchQuery({ infoUpdatedSince: "2010-12-1" }, cache, errors.throw, true)).toEqual({
    ...
    it("data-updated-since", function() {
      expect(api1.searchQuery({ "data-updated-since": "2020-01-01" }, cache, errors.throw, true)).toEqual({
        simfiles: { updatedAt: { $gte: new Date(2020, 0, 1) } },
      });
      expect(api1.searchQuery({ dataUpdatedSince: "2010-12-1" }, cache, errors.throw, true)).toEqual({
    ...
    it("has-data-files", function() {
      expect(api1.searchQuery({ "has-data-files": "true" }, cache, errors.throw, true)).toEqual({
        simfiles: { $exists: true },
      });
      expect(api1.searchQuery({ hasDataFiles: "false" }, cache, errors.throw, true)).toEqual({
    ...
    it("availability", function() {
      expect(api1.searchQuery({ availability: "available" }, cache, errors.throw, true)).toEqual({
        availability: { $in: [ 'regular', 'occasional' ] },
      });
      expect(api1.searchQuery({ availability: "regular" }, cache, errors.throw, true)).toEqual({
    ...
    it("max-results", function() {
      expect(api1.searchQuery({ type: "SU", impulseClass: "A", maxResults: 1 }, cache, errors.throw, true)).toEqual({
        type: "SU", impulseClass: "A",
      });
    });
    it("unknown", function() {
      expect(api1.searchQuery({ type: "SU", serialNumber: "34567" }, cache, errors.throw, false)).toEqual({
        type: "SU",
      });

      let errs = new errors.Collector();
      expect(api1.searchQuery({ type: "SU", serialNumber: "34567" }, cache, errs, true)).toEqual({
        type: "SU",
      });
      expect(errs.errorCount()).toBe(1);
      expect(errs.lastError().message).toBe('Invalid search criterion "serialNumber".');

      errs = new errors.Collector();
      expect(api1.searchQuery({ type: "SU", "serial-number": "34567" }, cache, errs, true)).toEqual({
        type: "SU",
      });
      expect(errs.errorCount()).toBe(1);
      expect(errs.lastError().message).toBe('Invalid search criterion "serial-number".');
    });
  });
