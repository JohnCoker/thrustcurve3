...
describe("errors", function() {
  ...
  describe("print", function() {
    ...
    it("missing param index", function() {
      ...
      }).toThrow('invalid message parameters: empty file {}');
    });
    it("invalid param index", function() {
      expect(function() {
        errors.print(errors.DATA_FILE_EMPTY, "empty file {0}", "foo.txt");
      }).toThrow('invalid message parameters: empty file {0}');
      expect(function() {
        errors.print(errors.DATA_FILE_EMPTY, "empty file {x}", "foo.txt");
      }).toThrow('invalid message parameters: empty file {x}');
      expect(function() {
        errors.print(errors.DATA_FILE_EMPTY, "empty file {1}, line {2}", "foo.txt");
      }).toThrow('invalid message parameter: {2}');
    });
    it("message", function() {
      errors.print(errors.DATA_FILE_EMPTY, "no stuff");
    });
    it("message w/ param", function() {
      errors.print(errors.DATA_FILE_FORMAT, "bad stuff in {1}", "foo.txt");
    });
    it("message w/ params", function() {
      errors.print(errors.DATA_FILE_FORMAT, "bad stuff in {1}, line {2}", "foo.txt", 17);
    });
  });
