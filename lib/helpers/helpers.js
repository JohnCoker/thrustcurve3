...
function isVerified(options) {
  let clsNamespace = getNamespace('session');
  let req = clsNamespace != null ? clsNamespace.get('req') : null;
  if (req && req.user && req.user.verified) {
    /* jshint validthis:true */
    return options.fn(this || module);
  }
}

function notVerified(options) {
  let clsNamespace = getNamespace('session');
  let req = clsNamespace != null ? clsNamespace.get('req') : null;
  if (req != null && req.user != null && !req.user.verified) {
    /* jshint validthis:true */
    return options.fn(this || module);
  }
}
...
function canCompare(options) {
  let clsNamespace = getNamespace('session');
  let req = clsNamespace != null ? clsNamespace.get('req') : null;
  if (req && req.session && req.session.motorsViewed && req.session.motorsViewed.length > 1) {
    /* jshint validthis:true */
    return options.fn(this || module);
  }
}

function recentSuffix() {
  let clsNamespace = getNamespace('session');
  let req = clsNamespace != null ? clsNamespace.get('req') : null;
  if (req && req.session && req.session.motorsViewed && req.session.motorsViewed.length > 0)
    return new handlebars.SafeString(' <b>(' + req.session.motorsViewed.length + ')</b>');
  return '';
}
...
module.exports = {
  ...
  /**
   * Format a number with the specified number of fraction digits for display.
   * @function
  ...
  /**
   * Block helper to check whether no user is verified.
   * @function
   */
  isVerified,

  /**
   * Block helper to check whether user is logged in, but not verified.
   * @function
   */
  notVerified,
  ...
  /**
   * Format a mass fraction for display.
   * @function
  ...
  /**
   * Block helper gating inner content on whether there are multiple motors recently viewed.
   * @function
   */
  canCompare: canCompare,

  /**
   * Produce a suffix showing how many files are recently viewed.
   * @function
   * @return {string} recent suffix
   */
  recentSuffix: recentSuffix,
