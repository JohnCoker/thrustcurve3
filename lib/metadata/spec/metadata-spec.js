...
describe("metadata", function() {
  ...
  describe("sample", function() {
    ...
    it("types", function() {
      expect(cache.types).toEqual(["SU", "reload"]);
    });
    it("diameters", function() {
      expect(cache.diameters.length).toBe(9);
      expect(cache.diameters[0]).toBe(13);
      expect(cache.diameters[8]).toBe(98);
    });
    it("impulseClasses", function() {
      expect(cache.impulseClasses.length).toBe(14);
      expect(cache.impulseClasses[0]).toBe("A");
      expect(cache.impulseClasses[13]).toBe("N");
    });
  });
});
