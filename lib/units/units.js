...
function formatUnit(v, type, u, fixed) {
  ...
  let fmt;
  if (d > 0)
    fmt = new Intl.NumberFormat('en-US', { minimumFractionDigits: d, maximumFractionDigits: d });
  else
    fmt = new Intl.NumberFormat('en-US', { maximumFractionDigits: 0 });
  s = fmt.format(v);
  if (!fixed)
    s = s.replace(/^([^.]*\.[0-9])0+$/, "$1");
  return s + SEP + unit.label;
}
...
module.exports = {
  ...
  /**
   * Separator (non-breaking space) between the number and unit.
   */
  SEP,
};
